    ...
    if st.button("🎯 Generate Image Prompt", type="primary"):
        ...
        if repopulate_rendered_people:
            prompt += (
                ...
                "- INSTRUCTION 1 — POSITION ANCHOR (EXECUTE FIRST, BEFORE ANY OTHER STEP): "
                "Before altering any person's appearance, scan the entire uploaded image and "
                "record the precise pixel-space location, bounding area, scale, and body "
                "orientation of every human figure. Treat these recorded positions as immovable "
                "fixed anchors. Every replacement figure must be centred on its corresponding "
                "anchor exactly. No figure may shift left, right, forward, backward, or change "
                "in scale by any amount. If a person's feet are at a specific point on the floor, "
                "the replacement figure's feet must contact the floor at that identical point. "
                "Position is locked. It cannot change under any circumstance.\n"

                "- INSTRUCTION 2 — COUNT LOCK: Count the exact number of human figures in the "
                "uploaded image. The output must contain that exact same number — no more, no less. "
                "Do NOT add any additional figures. Do NOT remove any existing figures. "
                "This count is fixed and cannot be changed.\n"
                ...
                "- FINAL VERIFICATION — before outputting, confirm: (a) the number of people "
                "matches the input exactly, (b) every person is in their original anchored "
                "position, (c) no figure has moved, been added, or been removed.\n"
