...
with tab1:
    ...
    if st.button("🎯 Generate Image Prompt", type="primary"):
        ...
        prompt += (
            "\n**[OUTPUT RESOLUTION & CLARITY]**\n"
            "- Render the output at the maximum possible output resolution and detail density. "
            "Do not allow any region of the image to be soft, blurred, or under-resolved unless "
            "it is deliberately within the depth-of-field falloff zone.\n"
            "- FACE CLARITY — CRITICAL: Every human face in the scene, regardless of how small "
            "it appears in the frame, must be rendered at maximum microdetail fidelity. "
            "Faces must never appear pixelated, smeared, blended, or soft. "
            "Each face should look as though it was photographed with a high-resolution medium "
            "format camera and then placed into the scene — sharp, three-dimensional, and "
            "anatomically resolved at the level of individual pores, eyelashes, and iris texture. "
            "If a face is small due to distance, it must still be sharp and readable — "
            "not a blurred approximation.\n"
            "- TEXTURE SHARPNESS: All surface materials — skin, fabric, stone, timber, metal, "
            "glass — must be rendered with micro-texture sharpness. No surface should appear "
            "smooth, waxy, or AI-generated. Every material has physical grain, weave, or "
            "granularity visible at full resolution.\n"
            "- EDGE DEFINITION: All edges throughout the image — architectural lines, hair "
            "strands, clothing seams, material transitions — must be crisp and physically "
            "accurate. No feathered, haloed, or blended edges except where natural depth of "
            "field dictates.\n"
            "- NOTE FOR POST-PROCESSING: If the source reference image is low resolution, "
            "the output should still be generated at maximum fidelity. After generation, "
            "for further face and detail enhancement, consider running the output through "
            "a dedicated AI upscaler such as Topaz Gigapixel AI, Adobe Firefly Enhance, "
            "or Google Photos Magic Eraser / Photo Unblur for final pixel-level refinement.\n"
        )
