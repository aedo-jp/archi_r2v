...
with tab1:
    ...
    if st.button("🎯 Generate Image Prompt", type="primary"):
        ...
        # ==========================================
        prompt = (
            "**[BASE CONSTRAINT — READ THIS FIRST]**\n"
            "- SCENE LOCK — NON-NEGOTIABLE: The uploaded image defines the scene. "
            "The architecture, the view, the camera position, the camera target, the horizon "
            "line, the framing, and every structural element in the composition are permanently "
            "fixed and must not change under any circumstance. This prompt does not ask for a "
            "new scene, a new viewpoint, or a reinterpretation. It asks for the exact uploaded "
            "scene to be retouched to photographic quality. If the uploaded image shows the "
            "north-facing facade from street level, the output shows the north-facing facade "
            ...
            f"- ABSOLUTE STANDARD: The output must be indistinguishable from an editorial "
            f"photograph published in Architectural Digest or Wallpaper* magazine. "
            f"Style: {photographer_style}.\n"
            "- The final image must read unmistakably as a photograph captured by a real camera "
            "of the exact scene shown in the reference — not a new illustration, a reinterpretation, "
            "or a different view of the same building.\n\n"
        )
        ...
        if repopulate_rendered_people:
            prompt += (
                ...
                "- INSTRUCTION 1 — COUNT LOCK (READ THIS BEFORE DOING ANYTHING ELSE): "
                "Look at the uploaded image. Count every human figure that is visible — "
                "including partial figures, figures in the background, and figures that are "
                "partly obscured. Write that number down internally. That number is now fixed. "
                "The output image must contain exactly that number of people — not one more, "
                "not one fewer. This is the single most important constraint in this entire "
                "prompt. Do NOT add any new people. Do NOT fill empty space with figures. "
                "Do NOT interpret any part of this prompt as a request to add people. "
                "The only task is to rephotograph the people already present at higher quality. "
                "Every other instruction below operates within this fixed count.\n"

                "- INSTRUCTION 2 — POSITION ANCHOR: Before altering any person's appearance, "
                "record the precise pixel-space location, bounding area, scale, and body "
                "orientation of every human figure in the uploaded image. Treat these recorded "
                "positions as immovable fixed anchors. Every replacement figure must be centred "
                "on its corresponding anchor exactly. No figure may shift left, right, forward, "
                "backward, or change in scale by any amount. If a person's feet are at a specific "
                "point on the floor, the replacement figure's feet must contact the floor at that "
                "identical point. Position is locked and cannot change under any circumstance.\n"

                "- INSTRUCTION 3 — DEMOGRAPHIC & APPEARANCE LOCK: For each figure, analyse their "
                "visual cues (hair length, hair colour, clothing style, skirts vs. trousers, "
                "body shape, apparent height and build) to determine their gender, approximate age, "
                "ethnicity, and skin tone. The replacement subject must match all of these "
                "characteristics precisely. Do not change gender, do not change skin tone, "
                "do not change approximate age group, do not change hair colour or length. "
                "Preserve the inferred clothing style and colour palette of each individual.\n"

                "- INSTRUCTION 4 — POSE & ORIENTATION LOCK: Each replacement figure must replicate "
                "the body orientation and facing direction of the original figure exactly. "
                "If the original figure faces away from camera, the replacement faces away. "
                "If the original is mid-stride with weight on the left foot, the replacement "
                "mirrors that posture. Do not invent new poses.\n"

                "- INSTRUCTION 5 — FACE QUALITY: Replace each original face with a photorealistic "
                "human face that matches the inferred demographic. Each face must exhibit: "
                "physically accurate subsurface scattering (soft light penetrating the dermis, "
                "warm glow at earlobes, nostrils, and fingertips); natural skin texture with "
                "visible pore structure and fine surface hairs; subtle natural anatomical asymmetry; "
                "specular catchlights in the eyes matching the dominant scene light source in "
                "position and colour; natural scleral vein detail and correct iris depth; "
                "natural lip moisture variation. No airbrushing, no smoothing, no uncanny valley "
                "symmetry — imperfection is authenticity.\n"

                "- INSTRUCTION 6 — SKIN & HAIR QUALITY: Skin must show physically accurate "
                "subsurface scattering with warm undertones visible where light transmits through "
                "thin areas (earlobes, nose tip, finger webbing). Natural colour variation across "
                "each face — ruddier at cheeks and nose, cooler at temples and jaw. "
                "Hair must be rendered at strand-level detail with natural sheen variation and "
                "correct translucency at backlit edges.\n"

                "- INSTRUCTION 7 — CLOTHING & GROUND CONTACT: Clothing must drape and fold "
                "naturally with wrinkles at joints (elbows, knees, waist). Each figure's feet "
                "must make genuine, physically accurate contact with the floor surface at their "
                "locked anchor position, with a correct contact shadow beneath.\n"

                "- FINAL VERIFICATION — before outputting, confirm all three: "
                "(a) count the people in the output — it must match the input count exactly; "
                "(b) every person occupies their original anchored position without any drift; "
                "(c) no figure has been moved, added, invented, or removed. "
                "If any of these three checks fail, correct the output before delivering it.\n"

                f"- LIGHTING INTEGRATION: All subjects are lit by the {clean_time.lower()} "
                "lighting environment. Shadow direction on every body matches the scene's primary "
        ...
        prompt += (
            "\n**[SHARPNESS & DETAIL FIDELITY]**\n"
            "- SCENE INTEGRITY: This is not a new image. It is a photographic retouching of the "
            "exact scene in the uploaded reference. Every element — the architecture, the view, "
            "the camera position, the horizon line, the framing — must remain absolutely identical "
            "to the reference. Only material surface quality, lighting fidelity, and human subject "
            "quality are being elevated. Nothing structural changes.\n"
            "- OVERALL SHARPNESS: Maximise detail resolution across every surface within the "
            "focal plane. No region inside the depth-of-field zone may appear soft, smeared, "
            "or under-resolved. Architectural edges must be crisp. Material grain must be visible "
            "at full resolution — timber, stone, metal, fabric, and glass all have physical "
            "micro-texture that must read clearly.\n"
            "- FACE & SKIN SHARPNESS: Human faces must be resolved at maximum detail within "
            "their position in the scene. Pore structure, eyelashes, iris texture, and lip "
            "detail must all be physically sharp and three-dimensional — not blended or softened. "
            "Faces that are smaller in the frame due to distance must still be as sharp as the "
            "scene's focal depth allows — never a blurred or pixelated approximation.\n"
            "- HAIR SHARPNESS: Individual hair strands must be resolved and separated — "
            "not merged into a single blended mass.\n"
            "- NO AI SOFTENING: Do not apply any smoothing, beautification, skin-softening, "
            "or noise reduction that reduces physical surface detail. Natural sensor grain "
            "in shadow areas is correct and must be retained. Artificial blurring or "
            "AI-style smoothing is strictly prohibited.\n"
        )
